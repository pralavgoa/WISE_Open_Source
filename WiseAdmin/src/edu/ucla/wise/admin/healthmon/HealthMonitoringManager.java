...
public class HealthMonitoringManager implements Runnable {
	...
	Session session = WISEApplication.get_mailSession(null);
	if (session == null) {
	...
	pass = WISEApplication.sharedProps.getString("wise.email.password");
	user = WISEApplication.sharedProps.getString("wise.email.username");//
