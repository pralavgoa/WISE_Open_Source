...
public class consent_generate extends HttpServlet
{
		            	...
		            	throws ServletException, IOException
	{
        ...
        if(pf!=null)
        {
            ...
            Consent_Form consent_f = pf.get_consent_form_survey_irb(theUser.currentSurvey.id, theUser.irb_id);
            if(consent_f == null)
            {
            ...
            consent_header += "<LINK href='"+ s.shared_file_url + "style.css' type=text/css rel=stylesheet>";
            ...
            consent_notes += "<A href=\"javascript:FormSubmit('yes')\"><IMG src='"+"imageRender?img=accept.gif' border=0></A>";
            consent_notes += "&nbsp;<A href=\"javascript:FormSubmit('no')\"><IMG src='"+"imageRender?img=decline.gif' border=0></A></DIV>";
            ...
            if(!title.equalsIgnoreCase(""))
              consent_title += "<B><FONT face='Arial, Helvetica, sans-serif' size=3>"+title+"</FONT></B>";
            if(!sub_title.equalsIgnoreCase(""))
              consent_title += "<B> &#8212; <BR><FONT face='Arial, Helvetica, sans-serif' size=2>"+sub_title+"</FONT></B><BR>";

            //add the custormerized html code if it exits
            if(!consent_header_html.equalsIgnoreCase(""))
                consent_html += "<tr><td align=center>"+consent_header_html+"</td></tr>";
                        
            if(!consent_title.equalsIgnoreCase(""))
                consent_html += "<tr><td align=center>"+consent_title+"</td></tr>";

            if(!consent_p.equalsIgnoreCase(""))
                consent_html += "<tr><td align=left>"+consent_p+"</td></tr>";

            if(!consent_ul.equalsIgnoreCase(""))
                consent_html += "<tr><td align=left>"+consent_ul+"</td></tr>";
            ...
            consent_html += "<tr><td><p align=left><font size=2><b>IRB Number: "+apr_numb+"<br>";
        ...
        {
            WISE_Application.log_error("WISE - CONSENT GENERATE: can't find the preface object", null);
        }
        return;
	}
