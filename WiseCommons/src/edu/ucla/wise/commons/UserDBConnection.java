...
public class UserDBConnection {
    ...
    private static String escapeQuotesInString(String inputString){
	if(inputString==null){
	    return "";
	}
	...
	for(int i=0;i<inputString.length();i++){
		...
		returnString.append('\\').append(inputString.charAt(i));
	    }
	    else{
	...
	mainTable_name = surveyID + DataBank.MainTableExtension;
	try {
	...
	try {
	    ...
	    boolean query_success = stmt.execute(sql);
	    if (!query_success)
		System.out.println("SQL query failure on selecting repeating projects!!! : " + sql);
	    ResultSet rs = stmt.getResultSet();
		...
		for (column_index = 1; column_index <= columns; column_index++) {
			...
			javascript_array_response
				.append("\"" + col_name + "\"");
			javascript_array_response.append(":");
			javascript_array_response.append("\"" + escapeQuotesInString(ans) + "\"");
		    } else {// dont add;
		    }
	    ...
	    System.out.println("The output for the select statement for projects: "+javascript_array_response.toString());
