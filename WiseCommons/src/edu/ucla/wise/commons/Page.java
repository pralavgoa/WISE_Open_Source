...
public class Page {
    ...
    public ArrayList<RepeatingItemSet> repeating_items = new ArrayList<RepeatingItemSet>();
	...
	try {
	    ...
	    items = new PageItem[page_item_count];
		...
		if (PageItem.IsPageItemNode(node1)) {
			...
			RepeatingItemSet repeating_set = RepeatingItemSet
				.MakeNewItem(node1);
    ...
    public String render_admin_page(StudySpace ss) {
	String s = "<html>";
	...
	s += "<script type='text/javascript' language='JavaScript1.1' src='"
		+ SurveyorApplication.shared_file_url + "survey.js'></script>";
	s += "</head>";
	...
	if (survey.is_last_page(id))
	    // Servlet to render
	    s += "<a href='" + WISEApplication.admin_server
	...
	if (survey.is_last_page(id)) {
	    s += "<br><a href='" + WISEApplication.admin_server
