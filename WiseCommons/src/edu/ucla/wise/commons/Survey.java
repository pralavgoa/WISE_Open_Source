...
public class Survey {
    ...
    public Survey(Document xml_doc, StudySpace ss) {
	try {
	    int numb_pages = 0;
	    total_item_count = 0;
	    study_space = ss;
    ...
    public ResponseSet get_response_set(String id) {
	ResponseSet a = (ResponseSet) response_sets.get(id);
	return a;
    }

    /** search by ID, return a specific Subject_Set */
    public SubjectSet get_subject_set(String id) {
	SubjectSet ss = (SubjectSet) subject_sets.get(id);
	return ss;
    }
     ...
     * repeating sets
     */
    public ArrayList<RepeatingItemSet> get_repeating_item_sets() {
	ArrayList<RepeatingItemSet> survey_repeating_item_sets = new ArrayList<RepeatingItemSet>();
	for (int page_i = 0; page_i < pages.length; page_i++) {
	    ArrayList<RepeatingItemSet> page_repeating_item_sets = pages[page_i]
		    .get_repeating_item_sets();

	    for (RepeatingItemSet page_repeat_set_instance : page_repeating_item_sets) {
