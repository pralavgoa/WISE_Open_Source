...
//the constructor for message_sequence
    public Reminder(Node n)
    {
       ...
       try
       {
            ...
            trigger_days = Integer.parseInt(n.getAttributes().getNamedItem("Trigger_Days").getNodeValue());
            max_count = Integer.parseInt(n.getAttributes().getNamedItem("Max_Count").getNodeValue()); 
            //System.out.println("max count=" + max_count);
       }
       catch (Exception e)
       {
            WISE_Application.log_error("WISE EMAIL - REMINDER: ID = "+id+"; Subject = "+subject+" --> "+e.toString(), null);
            return;
       }
    }
