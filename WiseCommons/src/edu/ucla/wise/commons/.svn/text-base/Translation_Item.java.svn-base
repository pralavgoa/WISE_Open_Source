    ...
    public Translation_Item(Node n, Survey s) 
    {
        try
        {
            ...
            { 
                ...
                //parse the stem - translated version (open question/closed question/question block)
                if (nc.getNodeName().equalsIgnoreCase("Stem"))
                {
                    stem = nc.getFirstChild().getNodeValue();
                    //Study_Util.email_alert("get the translated stem: " + stem);
                    /*
                    Transformer transformer = TransformerFactory.newInstance().newTransformer();
                    StringWriter sw = new StringWriter();
                    transformer.transform(new DOMSource(nc), new StreamResult(sw));
                    stem = sw.toString();
                    Study_Util.email_alert("get the translated stem: " + stem);
                    */
                }
                //parse the translated response option (closed question/question block)
        ...
        {
            WISE_Application.log_error("WISE - Translation Item: ID = "+id+"; Survey = "+s.id+"; Study = "+s.study_space.id+" --> "+e.toString(), null);
            return;
        }
    }
