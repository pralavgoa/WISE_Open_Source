...
/** constructor: parse a response set node from XML */
    public Welcome_Page(Node n, Preface p)
    {
        try
        {
            preface = p;
            ...
            if(node_child !=null)
                ...
                title = "";
            node_child = n.getAttributes().getNamedItem("BannerFileName");
            if(node_child !=null)
        ...
        {
            WISE_Application.log_error("WISE - WELCOME PAGE : ID = "+id+"; Preface Project name = "+p.project_name+"; --> "+e.toString(), null);
            return;
        }
    }
}
