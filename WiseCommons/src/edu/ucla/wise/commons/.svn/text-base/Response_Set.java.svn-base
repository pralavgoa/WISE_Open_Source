...
/**
This class contains an answer set called response set and all its possible answers
The closed question & question block contain this response set
*/
...
/** constructor: parse a response set node from XML */
    public Response_Set(Node n, Survey s)
    {
        try
        {
            survey = s;
        ...
        {
            WISE_Application.log_error("WISE - RESPONSE SET : ID = "+id+"; Survey = "+s.id+"; Study = "+s.study_space.id+" --> "+e.toString(), null);
            return;
        }
    }
...
/*
    public String print()
    {
        String s = "RESPONSE SET<br>";
        s += "ID: "+id+"<br>";
        s += "Levels: "+levels+"<br>";
        s += "StartValue: "+startvalue+"<br>";
        s += "Responses: <br>";
        for (int i = 0; i < responses.size(); i++)
            s += values.get(i)+":"+responses.get(i)+"<br>";
        s += "<p>";
        return s;
    }
*/
